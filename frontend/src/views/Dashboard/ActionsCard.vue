...
const validateFormData = () => {
  console.log('validate formData', formData.value);
  this.errors = [];
  ...
  if (!formData.value.applicantMobilePhoneNumber) {
    this.errors.push('Mobile Phone Number is required');
  }
  ...
  if (!formData.value.annualIncomeBeforeTax) {
    this.errors.push('Annual Income Before Tax is required');
  }

  if (!formData.value.incomingAddress) {
    this.errors.push('Incoming Address is required');
  }

  if (!formData.value.incomingDeposit) {
    this.errors.push('Incoming Deposit is required');
  }

  if (!formData.value.incomingPrice) {
    this.errors.push('Incoming Price is required');
  }

  if (!formData.value.incomingStampDuty) {
    this.errors.push('Incoming Stamp Duty is required');
  }
  ...
  if (!formData.value.loanDuration) {
    this.errors.push('Loan Duration is required');
  }

  if (!formData.value.monthlyExpenses) {
    this.errors.push('Monthly Expenses is required');
  }

  if (!formData.value.outgoingAddress) {
    this.errors.push('Outgoing Address is required');  
  }

  if (!formData.value.outgoingMortgage) {
    this.errors.push('Outgoing Mortgage is required');
  }

  if (!formData.value.outgoingValuation) {
    this.errors.push('Outgoing Valuation is required');
  }

  if (!formData.value.savingsContribution) {
    this.errors.push('Savings Contribution is required');
  }
